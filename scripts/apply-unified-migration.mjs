...
if (!parsed.hostname.endsWith(".neon.tech"))
  throw new Error(
    "Refusing to run because DATABASE_URL is not a Neon hostname.",
  );
...
try {
  const identity = await client.query(
    "select current_database() as database, current_user as role, current_setting('server_version') as postgres_version",
  );
  const predecessorPresence = await tablePresence(predecessorTables);
  const missingPredecessors = predecessorPresence
    .filter((row) => !row.present)
    .map((row) => row.name);
  if (missingPredecessors.length)
    throw new Error(
      `Missing predecessor tables: ${missingPredecessors.join(", ")}`,
    );
  ...
  if (!apply) {
    ...
    const missingColumns = columnRows.rows
      .filter((row) => !row.present)
      .map((row) => `${row.table_name}.${row.column_name}`);
    if (missingTables.length || missingColumns.length) {
      throw new Error(
        `Migration verification failed. Missing tables: ${missingTables.join(", ") || "none"}; missing columns: ${missingColumns.join(", ") || "none"}`,
      );
    }
    ...
    console.log(
      JSON.stringify({
        phase: "verified",
        applied: true,
        idempotencyPasses: verifyIdempotent ? 2 : 1,
        after,
        missingTables,
        missingColumns,
      }),
