...
export async function finalizeAndLockMemberMonth(input:{month:string;memberName:string;result:FinalKpiResult;actor:string;acknowledgementReason?:string|null}){
  ...
  return transaction(async(client)=>{
    ...
    const inserted=await client.query(`insert into public.monthly_member_kpi_results
      ...
      JSON.stringify(input.result.sourceIds),JSON.stringify(audit),JSON.stringify(input.result),input.actor]);
