...
export async function saveQualityReview(input: { workEventId: string; criteria: CriterionReview[]; status: "approved" | "excluded"; exclusionReason?: string | null; adminNote?: string | null; evidence?: Record<string, unknown>; reviewer: string }) {
  return transaction(async (client) => {
    const event = await client.query(`select id::text,work_type,unit_value from public.url_work_events where id=$1 and is_countable=true and content_kpi_eligible=true limit 1`, [input.workEventId]);
...
export async function listQualityReviewQueue(month: string) {
  ...
  const result = await query(`select e.id::text as work_event_id,e.project,e.member_name,e.work_type,e.work_date::text,
    ...
    where e.is_countable=true and e.content_kpi_eligible=true and e.work_date>=date_trunc('month',$1::date) and e.work_date<date_trunc('month',$1::date)+interval '1 month'
