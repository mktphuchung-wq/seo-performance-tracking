...
export async function getProjectKpiSettings(): Promise<ProjectKpiSettings[]> {
  const res = await query<any>(`with projects as (select project, count(*)::int active_urls from content_urls where coalesce(is_active,true)=true and nullif(project,'') is not null group by project)
    select p.project as project, p.active_urls, s.id, s.project_kpi_type, s.project_start_date, s.is_kpi_protection_enabled, s.performance_floor_pct, s.performance_cap_pct, s.min_coverage_required, s.min_eligible_urls, s.max_excluded_no_data_rate, s.allow_auto_floor_when_low_confidence, s.allow_auto_floor_when_partial_coverage, s.allow_auto_floor_when_high_no_data, s.require_pm_review_below_pct, s.pm_override_enabled, s.pm_override_adjusted_pct, s.pm_override_reason, s.notes, s.created_at, s.updated_at from projects p left join public.project_kpi_settings s on s.project=p.project order by p.project`);
