...
test("source reconciliation reuses the active canonical event before an inactive legacy duplicate", async () => {
  const repository = await readFile(
    new URL("../repositories/work-source-rows.ts", import.meta.url),
    "utf8",
  );
  assert.match(
    repository,
    /order by\s*\(project_id=\$1[^`]+\) desc,\s*\(unified_source_state='active'\) desc,\s*id\s*limit 1/i,
  );
});
