...
test("Neon connection strings use verify-full without dropping channel binding",()=>{
  const normalized=normalizePostgresConnectionString("postgresql://user:password@ep-example.us-east-2.aws.neon.tech/neondb?sslmode=require&channel_binding=require");
...
test("canonical data source reads the persisted quality-review note column",async()=>{
  const repository=await readFile(new URL("../repositories/data-source.ts",import.meta.url),"utf8");
