...
export type UrlSortKey = "url" | "project" | "content_worked_at" | "clicks" | "impressions" | "ctr" | "position" | "click_growth_pct" | "impression_growth_pct" | "growth_status" | "refreshed_at";
export type UrlSortDirection = "asc" | "desc";
type LegacyUrlSortKey = UrlSortKey | "ctr_asc" | "position_asc" | "position_desc" | "opportunity";
type SortableUrlPerformance = UrlPerformance & { content_worked_at?: string | null; click_growth_pct?: number | null; impression_growth_pct?: number | null; status?: string; refreshed_at?: string | null };
...
export function UrlTable({ rows, sort = "clicks", direction, basePath = "", preserve = {} }: { rows: UrlPerformance[]; sort?: LegacyUrlSortKey; direction?: UrlSortDirection; basePath?: string; preserve?: Record<string, string | undefined> }) {
  ...
  return <DataTableContainer><table className="w-full min-w-[1280px] table-auto text-[13px] sm:text-sm"><thead className="bg-slate-100 text-left"><tr>{header("url", "URL", "p-3")}{header("project", "Project")}<th>Member</th>{header("content_worked_at", "Date / Worked At")}{header("clicks", "Clicks")}{header("impressions", "Impr.")}{header("ctr", "CTR")}{header("position", "Pos.")}{header("click_growth_pct", "Click Growth")}{header("impression_growth_pct", "Impr. Growth")}{header("growth_status", "Status")}<th>Opportunity</th>{header("refreshed_at", "Refreshed")}</tr></thead><tbody>{sortedRows(rows, activeSort.key, activeSort.direction).map((row) => { const r = row as SortableUrlPerformance; return <tr className="border-t" key={r.id}><td className="w-[34rem] max-w-[34rem] p-3"><Link className="block truncate text-blue-700" title={r.url} href={`/url/${r.id}`}>{r.url}</Link>{r.warning && <div className="text-xs text-amber-700">{r.warning}</div>}</td><td>{r.project}</td><td>{r.member_name}</td><td>{fmtWorkedDate(r.content_worked_at)}</td><td>{fmtNum(r.clicks)}</td><td>{fmtNum(r.impressions)}</td><td>{fmtPct(r.ctr)}</td><td>{fmtPos(r.position)}</td><td><span className={getGrowthClassName(r.click_growth_pct)}>{r.click_growth_pct === undefined ? "—" : fmtGrowth(r.click_growth_pct)}</span></td><td><span className={getGrowthClassName(r.impression_growth_pct)}>{r.impression_growth_pct === undefined ? "—" : fmtGrowth(r.impression_growth_pct)}</span></td><td>{r.status ? <StatusBadge status={r.status} /> : "—"}</td><td><span className="rounded-full bg-slate-100 px-2 py-1 text-xs">{labelText(r.opportunity)}</span></td><td>{fmtDateTime(r.refreshed_at)}</td></tr>; })}{rows.length === 0 && <tr><td className="p-3 text-slate-500" colSpan={13}>No URLs found for this account.</td></tr>}</tbody></table></DataTableContainer>;
}
