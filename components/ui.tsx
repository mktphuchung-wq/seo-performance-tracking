...
export function Shell({ children, email, isAdmin }: { children: React.ReactNode; email?: string | null; isAdmin?: boolean }) {
  ...
  const navItems = [
    ...
    ...(!isAdmin ? [{ href: `/kpi-month/${currentMonth}`, label: "KPI tháng của tôi" }] : []),
    { href: "/url-data-source", label: "Nguồn dữ liệu URL" },
    ...(isAdmin ? [
      { href: "/member-insights", label: "Phân tích thành viên" },
      { href: "/admin", label: "Quản trị" },
      { href: "/admin/project-kpi-settings", label: "Cài đặt KPI dự án" },
      { href: "/admin/kpi-month", label: "KPI tháng v2" },
    ] : []),
  ];
      ...
      <PageContainer className="py-4">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div className="min-w-0">
            <h1 className="text-2xl font-bold tracking-tight text-slate-950 sm:text-3xl">Theo dõi hiệu suất đội SEO</h1>
          </div>
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between xl:justify-end">
            ...
            {email && <div className="flex flex-wrap items-center gap-3 text-sm lg:justify-end">
              ...
              <a className="font-semibold text-blue-700 hover:text-blue-900" href="/api/auth/signout">Đăng xuất</a>
            </div>}
          </div>
        </div>
      </PageContainer>
...
export function WarningList({ warnings }: { warnings: (string | undefined)[] }) {
  const unique = Array.from(new Set(warnings.filter(Boolean).map((warning) => warning === "pending_refresh" ? "Chưa được làm mới" : warning)));
  if (!unique.length) return null;
  return <div className="mb-6 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><strong>Cảnh báo thiết lập:</strong><ul className="mt-2 list-disc pl-5">{unique.map((w) => <li key={w}>{w}</li>)}</ul></div>;
}
...
function fmtDateTime(value?: string | null) { if (!value) return "Chưa được làm mới"; const date = new Date(value); return Number.isNaN(date.getTime()) ? "—" : date.toLocaleString("vi-VN"); }
function fmtWorkedDate(value?: string | null) { if (!value) return "Chưa có ngày thực hiện"; const date = new Date(`${String(value).slice(0, 10)}T00:00:00.000Z`); return Number.isNaN(date.getTime()) ? "Chưa có ngày thực hiện" : date.toISOString().slice(0, 10); }
export function contentTypeLabel(value?: string | null) { const labels: Record<string, string> = { new_content: "Nội dung mới", audit: "Audit", update: "Cập nhật", portfolio: "Danh mục" }; return value ? (labels[value] ?? value.replace(/_/g, " ")) : "Chưa có loại"; }
export function UrlTable({ rows, sort = "clicks", direction, basePath = "", preserve = {} }: { rows: UrlPerformance[]; sort?: LegacyUrlSortKey; direction?: UrlSortDirection; basePath?: string; preserve?: Record<string, string | undefined> }) {
  ...
  return <DataTableContainer><table className="w-full min-w-[1360px] table-auto text-[13px] sm:text-sm"><thead className="bg-slate-100 text-left"><tr>{header("url", "URL", "p-3")}{header("project", "Dự án")}<th>Thành viên</th>{header("content_worked_at", "Ngày thực hiện")}{header("content_type", "Loại")}{header("clicks", "Lượt nhấp")}{header("impressions", "Hiển thị")}{header("ctr", "CTR")}{header("position", "Vị trí")}{header("click_growth_pct", "Tăng trưởng lượt nhấp")}{header("impression_growth_pct", "Tăng trưởng hiển thị")}{header("growth_status", "Trạng thái")}<th>Cơ hội</th>{header("refreshed_at", "Làm mới lúc")}</tr></thead><tbody>{sortedRows(rows, activeSort.key, activeSort.direction).map((row) => { const r = row as SortableUrlPerformance; return <tr className="border-t" key={r.id}><td className="w-[34rem] max-w-[34rem] p-3"><Link className="block truncate text-blue-700" title={r.url} href={`/url/${r.id}`}>{r.url}</Link>{r.warning && <div className="text-xs text-amber-700">{r.warning}</div>}</td><td>{r.project}</td><td>{r.member_name}</td><td>{fmtWorkedDate(r.content_worked_at)}</td><td><span className="rounded-full bg-blue-50 px-2 py-1 text-xs font-medium text-blue-800">{contentTypeLabel(r.content_type)}</span></td><td>{fmtNum(r.clicks)}</td><td>{fmtNum(r.impressions)}</td><td>{fmtPct(r.ctr)}</td><td>{fmtPos(r.position)}</td><td><span className={getGrowthClassName(r.click_growth_pct)}>{r.click_growth_pct === undefined ? "—" : fmtGrowth(r.click_growth_pct)}</span></td><td><span className={getGrowthClassName(r.impression_growth_pct)}>{r.impression_growth_pct === undefined ? "—" : fmtGrowth(r.impression_growth_pct)}</span></td><td>{r.status ? <StatusBadge status={r.status} /> : "—"}</td><td><span className="rounded-full bg-slate-100 px-2 py-1 text-xs">{labelText(r.opportunity)}</span></td><td>{fmtDateTime(r.refreshed_at)}</td></tr>; })}{rows.length === 0 && <tr><td className="p-3 text-slate-500" colSpan={14}>Không tìm thấy URL nào cho tài khoản này.</td></tr>}</tbody></table></DataTableContainer>;
}
...
function statusLabel(status?: string | null) { return uiLabel(status || "insufficient_data"); }
function rangeMetric(finalPerformance: MemberPerformanceFinalSummary | null | undefined, key: "1m" | "3m" | "6m" | "all_time", weight: string) {
  ...
  return { label: `KPI ${key.toUpperCase()}`, value: <div><div>{fmtKpi(value)}</div><div className="mt-2 text-sm font-normal text-slate-600">Trọng số {weight}{key !== "all_time" ? ` · URL đủ điều kiện ${eligible ?? 0} · URL thiếu dữ liệu đã loại ${excluded ?? 0}` : " · làm mới tùy chọn"}</div>{value == null && <div className="mt-2 text-xs font-medium text-slate-500">Được loại khỏi KPI cuối cùng và các trọng số sẽ được chuẩn hóa.</div>}</div>, tone: kpiTone(value) };
}
...
export function PerformanceKpiPanel({ finalPerformance, memberName, helper }: { finalPerformance?: MemberPerformanceFinalSummary | null; memberName?: string; helper?: string }) {
  ...
  return <section className={`rounded-2xl border p-5 shadow-sm sm:p-6 ${metricToneStyles[kpiTone(adjustedValue)].card}`}>
    <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-sm font-semibold uppercase tracking-wide text-slate-500">Performance cuối cùng ban đầu %</p><h3 className={`mt-1 text-4xl font-bold ${metricToneStyles[kpiTone(rawValue)].value}`}>{fmtKpi(rawValue)}</h3><p className="mt-3 text-sm font-semibold uppercase tracking-wide text-slate-500">Performance cuối cùng sau điều chỉnh %</p><h3 className={`mt-1 text-3xl font-bold ${metricToneStyles[kpiTone(adjustedValue)].value}`}>{fmtKpi(adjustedValue)}</h3>{adjustedValue != null && rawValue != null && adjustedValue > rawValue && <p className="mt-2 rounded-lg bg-blue-50 p-2 text-sm font-medium text-blue-800">Được điều chỉnh tăng theo quy tắc bảo vệ KPI dự án.</p>}{memberName && <p className="mt-1 text-sm text-slate-600">{memberName}</p>}</div><div className="grid gap-2 text-sm sm:grid-cols-3 lg:min-w-[520px]"><div className="rounded-xl bg-white/70 p-3"><span className="block text-xs uppercase text-slate-500">Trạng thái điều chỉnh</span><strong>{statusLabel(finalPerformance?.adjustment_status || finalPerformance?.performance_final_status)}</strong></div><div className="rounded-xl bg-white/70 p-3"><span className="block text-xs uppercase text-slate-500">Loại KPI dự án</span><strong>{statusLabel(finalPerformance?.project_kpi_type || "growth_project")}</strong></div><div className="rounded-xl bg-white/70 p-3"><span className="block text-xs uppercase text-slate-500">Cần PM đánh giá</span><strong>{yesNo(Boolean(finalPerformance?.pm_review_required))}</strong></div><div className="rounded-xl bg-white/70 p-3"><span className="block text-xs uppercase text-slate-500">Độ tin cậy</span><strong>{finalPerformance?.performance_confidence ? uiLabel(finalPerformance.performance_confidence) : "Mẫu dữ liệu nhỏ"}</strong></div><div className="rounded-xl bg-white/70 p-3"><span className="block text-xs uppercase text-slate-500">Độ phủ</span><strong>{finalPerformance ? fmtPct(finalPerformance.performance_final_coverage) : "—"}</strong></div><div className="rounded-xl bg-white/70 p-3"><span className="block text-xs uppercase text-slate-500">Trạng thái ban đầu</span><strong>{statusLabel(finalPerformance?.performance_final_status)}</strong></div></div></div>
    <p className="rounded-xl border border-white/70 bg-white/70 p-3 text-sm text-slate-700"><strong>Công thức:</strong> KPI cuối cùng sử dụng trọng số do quản trị viên cấu hình (mặc định 1M × 30% + 3M × 40% + 6M × 20% + all_time × 10%). Các khoảng thiếu hoặc chưa đủ dữ liệu được loại khỏi KPI cuối cùng và trọng số khả dụng sẽ được chuẩn hóa.</p>
...
export function StatusBadge({ status }: { status: string }) {
  ...
  const label = status === "new_signal" ? "Tăng trưởng mới" : status === "no_data" ? "Chưa đủ dữ liệu — đã loại khỏi KPI Performance" : uiLabel(status);
