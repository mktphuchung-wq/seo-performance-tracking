...
export function AdminDataControls({ range = "current_month", startDate, endDate }: { range?: string; startDate?: string; endDate?: string }) {
  ...
  return <div className="mb-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
    <div className="flex flex-wrap gap-3">
      <button className="rounded-lg bg-emerald-700 px-4 py-2.5 text-sm font-semibold text-white shadow-sm disabled:opacity-60" onClick={syncSheet} disabled={!!loading}>{loading === "sync" ? "Syncing URLs…" : "Sync URLs from Sheet"}</button>
      {([
