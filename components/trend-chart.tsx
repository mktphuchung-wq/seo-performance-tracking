...
export function TrendChart({ data }: { data: DailyMetric[] }) {
  return <div className="h-72 rounded-xl border bg-white p-4">{data.length ? <ResponsiveContainer width="100%" height="100%"><LineChart data={data}><XAxis dataKey="date" /><YAxis /><Tooltip /><Line type="monotone" dataKey="clicks" name="Lượt nhấp" stroke="#2563eb" /><Line type="monotone" dataKey="impressions" name="Lượt hiển thị" stroke="#16a34a" /></LineChart></ResponsiveContainer> : <p className="text-slate-500">Chưa có dữ liệu theo ngày.</p>}</div>;
}
