...
export default async function AdminDashboard({ searchParams }: { searchParams?: { range?: string; startDate?: string; endDate?: string; sort?: import("../../../components/ui").UrlSortKey } }) {
  ...
  <h3 className="mb-3 mt-8 text-xl font-semibold">Member performance score</h3><div className="overflow-hidden rounded-xl border bg-white"><table className="w-full text-sm"><thead className="bg-slate-100 text-left"><tr><th className="p-3">Member</th><th>Score</th><th>Support signal</th><th>URLs</th><th>Clicks</th><th>Growth</th><th>Growing</th><th>Declining</th></tr></thead><tbody>{members.slice(0,8).map(m=><tr className="border-t" key={m.member_name}><td className="p-3"><Link className="text-blue-700" href={`/admin/members/${encodeURIComponent(m.member_name)}`}>{m.member_name}</Link></td><td>{m.score}</td><td>{m.supportSignal}</td><td>{m.urlCount}</td><td>{fmtNum(m.clicks)}</td><td>{fmtGrowth(m.click_growth_pct)}</td><td>{m.growing}</td><td>{m.declining}</td></tr>)}</tbody></table><p className="p-3 text-xs text-slate-500">Performance score is an internal comparative score, not an absolute SEO quality score.</p></div>
  <Section title="Project growth insights" rows={group(performance, "project")} /><Section title="Performance by member" rows={group(performance, "member_name")} /><h3 className="mb-3 mt-8 text-xl font-semibold">Best growth URLs</h3><UrlTable rows={bestGrowth} /><h3 className="mb-3 mt-8 text-xl font-semibold">Declining URLs</h3><UrlTable rows={declining} /></Shell>;
}
