...
export async function GET() {
  try {
    ...
    return NextResponse.json({ ok: schema.ok && contentWorkedAtExists, database: { connected: true }, schema, diagnostics: {
      ...
      contentKpiEligibleWorkEvents: await safeCount("select count(*) from public.url_work_events where content_kpi_eligible=true and is_countable=true"),
      performanceKpiEligibleWorkEvents: await safeCount("select count(*) from public.url_work_events where performance_kpi_eligible=true and is_countable=true"),
