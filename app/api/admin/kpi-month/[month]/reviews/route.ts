...
export async function GET(_request:Request, props:{params: Promise<{month:string}>}) {
  ...
  const session=await getServerSession(authOptions);if(!session?.user?.email||!session.user.isAdmin)return NextResponse.json({error:"Forbidden"},{status:403});return NextResponse.json({reviews:await listQualityReviewQueue(/^\d{4}-\d{2}$/.test(params.month)?`${params.month}-01`:params.month)});
}
