  ...
  return <Shell email={session.user.email} isAdmin={session.user.isAdmin}>
    ...
    {selectedSetting && <form action="/api/project-kpi-settings" method="post" className="mb-8 rounded-2xl border bg-white p-5 shadow-sm">
      ...
      <div className="grid gap-6">
        ...
        <section><h4 className="mb-3 font-semibold text-slate-800">Not Enough Data Policy</h4><div className="grid gap-4 md:grid-cols-3"><label className="text-sm text-slate-600">Not Enough Data Policy<select className="mt-1 w-full rounded-lg border px-3 py-2" name="not_enough_data_policy" defaultValue={selectedSetting.not_enough_data_policy}>{noDataPolicies.map((policy) => <option key={policy} value={policy}>{policy.replace(/_/g, " ")}</option>)}</select></label>{field("neutral_no_data_score_pct", "Neutral No-data Score %", selectedSetting.neutral_no_data_score_pct, "number")}{field("min_url_age_days_for_penalty", "Min URL Age Days For Penalty", selectedSetting.min_url_age_days_for_penalty, "number")}{field("max_no_data_penalty_pct", "Max No-data Penalty %", selectedSetting.max_no_data_penalty_pct, "number")}{field("no_data_rate_pm_review_pct", "No-data Rate Requiring PM Review %", selectedSetting.no_data_rate_pm_review_pct, "number")}</div><p className="mt-2 text-xs text-slate-500">Not enough data is not poor performance by default; choose whether to exclude, score neutral, mildly penalize stable projects, or require PM review.</p></section>
